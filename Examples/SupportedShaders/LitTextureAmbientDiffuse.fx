...
// === Lighting Parameters ===
float3 LightDirection;        // Must be normalized before use
float4 AmbientColor;          // e.g., (0.2, 0.2, 0.2, 1.0)
float AmbientIntensity;       // e.g., 0.5
float4 DiffuseColor;          // e.g., (1.0, 1.0, 1.0, 1.0)
float DiffuseIntensity;       // e.g., 1.0
Texture2D Texture;
...
float4 PS(PixelInput input) : SV_Target
{
    ...
    // Compute diffuse factor using Lambert's cosine law
    ...
    float4 diffuse = DiffuseColor * DiffuseIntensity * NdotL;

    // Sample texture
    float4 texColor = Texture.Sample(samplerState, input.TexCoord);

    // Combine lighting with vertex color and texture
    float4 finalColor = (ambient + diffuse) * texColor * input.Color;

    return finalColor;
}

// === TECHNIQUE ===
technique AmbientDiffuseTextureColor {
    pass P0 {
